...
export function InteractiveSeatMapModal() {
  ...
  return (
    ...
    >
      <div
        className="relative w-full max-w-4xl bg-[#121215] border border-neutral-800 rounded-xl shadow-2xl overflow-hidden flex flex-col max-h-[92vh]"
        onClick={(e) => e.stopPropagation()}
      >
        {/* Header */}
        <div className="p-4 sm:p-6 border-b border-neutral-800 bg-neutral-900/60 flex items-start justify-between">
          <div>
            <div className="flex items-center gap-2 mb-1">
              <span className="px-2.5 py-0.5 text-xs font-semibold rounded bg-neutral-800 text-neutral-300 border border-neutral-700">
                {showtime.format}
              </span>
        ...
        <div className="flex-1 overflow-y-auto p-4 sm:p-8 flex flex-col items-center">
          {/* Subtle Curved Screen Graphic */}
          <div className="w-full max-w-lg mb-8 text-center">
            <div className="relative h-6 w-full overflow-hidden">
              <div className="absolute inset-x-8 top-0 h-8 border-t-2 border-neutral-600 rounded-[100%]" />
            </div>
            <p className="text-[10px] tracking-[0.25em] text-neutral-400 uppercase font-medium">
              Auditorium Screen
            </p>
          </div>
            ...
            {['A', 'B', 'C', 'D', 'E', 'F', 'G', 'H'].map((rowLetter) => {
              ...
              return (
                <div key={rowLetter} className="flex items-center gap-2 sm:gap-3">
                  <span className="w-4 text-xs font-semibold text-neutral-500 text-center select-none">
                    {rowLetter}
                  </span>
                  ...
                  <div className="flex items-center gap-1.5 sm:gap-2">
                    {rowSeats.map((seat) => {
                      ...
                      if (seat.tier === 'Prime Center') {
                        colorClasses = 'bg-neutral-700/80 border-neutral-600 hover:border-neutral-400 text-neutral-200';
                      } else if (seat.tier === 'Luxury Recliner') {
                        colorClasses = 'bg-neutral-600/80 border-neutral-500 hover:border-neutral-400 text-neutral-100';
                      }

                      if (isSelected) {
                        colorClasses = 'bg-white border-white text-black font-bold scale-105';
                      } else if (isReserved) {
                      ...
                      return (
                        ...
                        >
                          {isSelected ? <Check className="w-3.5 h-3.5 text-black" /> : seat.number}
                        </button>
                      );
                    })}
                  </div>
        ...
        <div className="p-4 sm:p-5 border-t border-neutral-800 bg-neutral-950 flex flex-col sm:flex-row items-center justify-between gap-4">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <div className="flex flex-wrap gap-1">
                    {selectedSeats.map((s) => (
                      <span
                        key={s.id}
                        className="px-2 py-0.5 rounded bg-neutral-800 text-neutral-200 text-xs font-semibold border border-neutral-700"
                      >
                        {s.id}
                      </span>
                    ))}
          ...
          <div className="flex items-center gap-3 w-full sm:w-auto">
            <button
              ...
              className="flex-1 sm:flex-none px-4 py-2 rounded-lg border border-neutral-700 hover:border-neutral-500 text-neutral-300 hover:text-white text-xs font-medium transition-colors"
            >
            ...
            <button
              ...
              className={`flex-1 sm:flex-none px-5 py-2 rounded-lg text-xs font-semibold flex items-center justify-center gap-2 transition-colors ${
                selectedSeats.length > 0
